...
class HS2Data(object):
        ...
        try:
            ...
            if np.shape(myshape)==(1,):   
                for i_mode in range(0,num_modes):
                    ...
                    self.modes.damping  [i_mode,:,1] = hs2cmd[i_mode + num_modes + 1]
                    self.modes.realpart [i_mode,:,0] = hs2cmd[0         ]
                    self.modes.realpart [i_mode,:,1] = hs2cmd[i_mode + 2*num_modes + 1]
            else:    
                for i_mode in range(0,num_modes):
                    self.modes.frequency[i_mode,:,0] = hs2cmd[:,0         ]
                    self.modes.frequency[i_mode,:,1] = hs2cmd[:,i_mode+  1]
    ...
    def read_op_data(self):
        ...
        try:
            ...
            print('ERROR: HAWCStab2 op file %s not found! Abort!'%self.filenameop)
            raise
...
class HS2BladeOnlyData(HS2Data):
