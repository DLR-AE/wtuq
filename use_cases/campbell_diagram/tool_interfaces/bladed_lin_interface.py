...
class BladedLinModel(BladedModel):
    ...
    def extract_results(self):
        ...
        result_dict['mode_names'] = mode_names

        result_dict['total_series'] = np.concatenate((result_dict['Frequency'].flatten(), result_dict['Damping'].flatten()))

        # save operational data
        result_dict['Nominal wind speed at hub position'] = bladed_result['Nominal wind speed at hub position']
        result_dict['Nominal pitch angle'] = bladed_result['Nominal pitch angle']
        result_dict['Rotor speed'] = bladed_result['Rotor speed']
        result_dict['Electrical power'] = bladed_result['Electrical power']

        if False:
            from campbellviewer.interfaces.bladed import BladedLinData
            cv_bladedlin_data = BladedLinData(self.run_directory, self.result_prefix)
            cv_bladedlin_data.read_data()
            result_dict['cv_bladedlin_data'] = cv_bladedlin_data
        ...
        np.savetxt(os.path.join(self.run_directory, 'modified.opt'), opt_data, header='wind speed, pitch, rpm, power, thrust')
        '''
