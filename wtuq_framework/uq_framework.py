...
def morris_screening(self, **kwargs):
    ...
    sample = sampler.random(n=nr_of_repetitions).T
    ...
    # ...
    all_samples = np.repeat(sample, nr_params+1, axis=1)
    ...
    for repetition_idx in range(nr_of_repetitions):
        for param_idx in range(nr_params):
            all_samples[param_idx, repetition_idx*(nr_params+1) + param_idx + 1] = all_samples[param_idx, repetition_idx*(nr_params+1) + param_idx + 1] + disturbance_term_each_param[param_idx]
